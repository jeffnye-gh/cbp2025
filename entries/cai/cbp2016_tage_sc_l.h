...
struct cbp_hist_t
{
    ...
    bool tage_pred;     // unused right now
#ifdef LOOPPREDICTOR
...
public:
    ...
    void init_histories(cbp_hist_t &current_hist)
    {
        ...
        for (int i = 1; i <= NHIST; i++)
        {
            ...
            logg[i] = (i < BORN) ? LOGG_LOW : LOGG_HIGH;
        }
    ...
    void Tagepred(UINT64 PC, const cbp_hist_t &hist_to_use)
    {
        ...
        for (int i = 1; i <= NHIST; i += 2)
        {
            ...
            GI[i + 1] = (i < BORN) ? GI[i] ^ (GTAG[i] & ((1 << LOGG_LOW) - 1)) : GI[i] ^ (GTAG[i] & ((1 << LOGG_HIGH) - 1));
        }
    ...
    // modification - pass in latency of branch as well
    ...
    // drop checkpoints when not updating tage
    void kill_checkpoint(uint64_t seq_no, uint8_t p)
    {
        const auto pred_hist_key = get_unique_inst_id(seq_no, p);
        pred_time_histories.erase(pred_hist_key);
    }
    ...
    bool getloop(UINT64 PC, const cbp_hist_t &hist_to_use)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (ltable[index].TAG == LTAG)
            {
                ...
                // if (LVALID && PC == 0x0ffff7fc542c0) { printf("LVALID AT PRED: NbIter: %d\n", ltable[index].NbIter); }
                // if (LVALID && PC == 0x0ffff7fc542c0 && ltable[index].NbIter != 33) { printf("LOOP IS WRONG SOMEHOW, NbIter is %d\n", ltable[index].NbIter); }
    ...
    void loopupdate(UINT64 PC, bool Taken, bool ALLOC, std::vector<lentry> &ltable)
    {
        if (LHIT >= 0)
        {
            ...
            if (LVALID)
            {
                if (Taken != predloop)
                {
                    // if (PC == 0x0ffff7fc542c0) { printf("LOOP MISPRED on %lu, NbIter is %d, Taken was %d, we predicted %d\n", hist_to_use.TEST_SANITY, ltable[index].NbIter, Taken, hist_to_use.predloop); }
                    // printf("\nDEALLOCATING\n");
                    //  free the entry
            ...
            if (Taken != ltable[index].dir)
            {
                ...
                else
                {
                    if (ltable[index].NbIter == 0)
                    {
                        ...
                        ltable[index].SpecCurrentIter %= ltable[index].NbIter;
                    }
                    else
                    {
