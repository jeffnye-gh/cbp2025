...
//The base table  in the SC component indexed with only PC + information 
// flowing out from  TAGE
// In order to  allow computing SCSUM in parallel with TAGE check, only 
// LongestMatchPred and HCpred are used. 4 SCSUM are computed, and a 
// final 4-to-1 selects the correct prediction:   each extra bit of 
// information (confidence, etc) would necessitate  doubling the number 
// of computed SCSUMs and double the width of the final MUX
...
// a single  physical table but  two logic tables: indices 
// agree on all the bits except 2
...
// second local history:in fact history within the same instruction 
...
//fourth local history  incorporates 2 bits of global history and 
//the current direction in the "local history"
...
//provide extra multiplicative weight to emphasise/deemphasize 
//the contribution of IMLI and local components
...
// we use a 8K entries history buffer to store the branch history: 
// 5000 are needed in the simulator + 5 * the number of inflight branches
...
//the counter(s) to chose between longest match and alternate 
// prediction on TAGE when weak counters: only plain TAGE
#define ALTWIDTH 5
...
struct cbp_hist_t
{
  ...
  // Needs for computing the "histories" for IMLI and backward/forward 
  // histories
  uint64_t  LastBack;
  uint64_t  LastBackPC;
  ...
  uint64_t TaIMLI;		// use to monitor the iteration number 
                      // (based on target locality for backward branches)
                      // Region 64 bytes
  uint64_t BrIMLI;		// use to monitor the iteration number (a second 
                      // version based on backward branch locality)) 
                      // Region 64 bytes
  uint64_t F_TaIMLI;	// use to monitor the iteration number,
                      // BHIST if TaIMLI = 0
  uint64_t F_BrIMLI;  // use to monitor the iteration number
                      // (a second version), FHIST if BrIMLI = 0

  uint64_t TaSIMLI;		// use to monitor the iteration number (based on 
                      // target locality for backward branches) Region 4 bytes
  uint64_t BrSIMLI;		// use to monitor the iteration number (a second 
                      // version based on backward branch locality))
                      // Region 4 bytes
  uint64_t F_TaSIMLI; // use to monitor the iteration number,
                      // BHIST if TaSIMLI = 0 
  uint64_t F_BrSIMLI; // use to monitor the iteration number
                      // (a second version), FHIST if BrSIMLI = 0 
...
int predictorsize ()
{
  ...
  //counters COUNT50 COUNT16_31 COUNT8_31 Count50 Count16_31 Count8_15
  STORAGESIZE += 3 * 7 * ((NHIST/4) * (1<<LOGCOUNT) +1);
  ...
  fprintf (stderr, " (TAGE %d) ", STORAGESIZE);
    ...
    fprintf(stderr,"(SC %d)", inter); inter =0;
      ...
      //The two forms of IMLI
        ...
        inter += 14;			// SLastBack
        #ifdef EXTRAW
          // the 2 EXTRAW   table multipliers
          inter += 2* PERCWIDTH * (1 << LOGSIZEUPS);
        #endif
        ...
        #ifdef LOCALS
          inter += (SNB ) * (1 << (LOGSNB)) * (PERCWIDTH);
          inter += NSECLOCAL * (Sm[0]);
        #endif //LOCALS

        #ifdef LOCALT
          inter += (QNB ) * (1 << (LOGQNB)) * (PERCWIDTH) ;
          inter += NQLOCAL * Qm[0];
          inter += (TNB ) * (1 << (LOGTNB)) * (PERCWIDTH) ;
          inter += NTLOCAL * Tm[0];
        #endif LOCALT
...
#define NNHIST 36
  ...
  bool predict_using_given_hist (uint64_t  seq_no, uint8_t piece, uint64_t  PCBRANCH, const cbp_hist_t& hist_to_use, const bool pred_time_predict)
  {
    ...
    SUMSC = 0;
    ////// These extra counters seem to bring a marginal  
        // gain of 0.006 MPKI  when only pure SC, not useful when other info
#ifndef SCMEDIUM
